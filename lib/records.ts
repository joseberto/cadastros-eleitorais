...
export function formatIndication(v?: string | null): string {
...
export function validate(input: unknown): RecordData {
    ...
    if (data.indication) data.indication = formatIndication(data.indication);
    return data;
}
...
export function normalizeImportRecord(input: unknown): RecordData & { marked?: boolean } {
    ...
    if (out.indication) out.indication = formatIndication(out.indication);
