...
import {Plus,Printer,ArrowDownAZ,ArrowUpAZ,UsersRound,Pencil,RefreshCw,ArrowLeft,FileText,ShieldCheck,X,Upload,FileJson,AlertTriangle,CheckCircle2} from 'lucide-react';
...
export default function Cadastros() {
 ...
 async function chooseImport(file?:File){
  ...
  if(file.size>2_000_000){setImportError('O arquivo ultrapassa 2 MB.');setImportOpen(true);return;}
  try{
   const payload=JSON.parse(await file.text()) as unknown; const list=extractImportRecords(payload); if(!list.length)throw new Error('O JSON não contém cadastros.'); if(list.length>1000)throw new Error('O limite é de 1.000 cadastros por arquivo.');
   const known=new Set(records.map(recordIdentity).filter(Boolean)); const rows:ImportRow[]=[];
   list.forEach((raw,index)=>{try{const data=validate(normalizeImportRecord(raw));const key=recordIdentity(data);const duplicate=!!key&&known.has(key);if(key)known.add(key);rows.push({index:index+1,data,duplicate});}catch(e){rows.push({index:index+1,error:(e as Error).message});}});
 ...
 async function runImport(){
  const ready=importRows.filter(r=>r.data&&!r.duplicate).map(r=>r.data as RecordData); if(!ready.length)return; setImporting(true);setImportError('');
  try{const r=await fetch('/api/records/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({records:ready})});const j=await r.json() as {error?:string;created?:Person[];createdCount?:number;duplicateCount?:number;invalidCount?:number};if(!r.ok)throw new Error(j.error);const created=j.created||[];setRecords(old=>[...old,...created]);setImportOpen(false);setImportRows([]);setNotice(`${j.createdCount??created.length} cadastro(s) importado(s) com sucesso.${(j.duplicateCount||0)>0?' '+j.duplicateCount+' repetido(s) ignorado(s).':''}`);}
 ...
 return <>
  ...
  <Dialog open={importOpen} onOpenChange={v=>{if(!importing)setImportOpen(v);}}><DialogContent className="import-modal" showCloseButton={false}><DialogHeader><div className="modal-heading"><div><DialogTitle>Importar cadastros por JSON</DialogTitle><DialogDescription>{importName?<>Arquivo: <strong>{importName}</strong></>:<>Selecione um arquivo JSON com os cadastros.</>}</DialogDescription></div><Button variant="ghost" size="icon" aria-label="Fechar importação" disabled={importing} onClick={()=>setImportOpen(false)}><X/></Button></div></DialogHeader>
   ...
   {!!importRows.length&&<><div className="import-summary"><span><FileJson size={18}/><strong>{importRows.length}</strong> no arquivo</span><span className="ok"><CheckCircle2 size={18}/><strong>{importRows.filter(r=>r.data&&!r.duplicate).length}</strong> prontos</span><span><strong>{importRows.filter(r=>r.duplicate).length}</strong> repetidos</span><span className={importRows.some(r=>r.error)?'warn':''}><AlertTriangle size={18}/><strong>{importRows.filter(r=>r.error).length}</strong> inválidos</span></div><p className="import-help">Registros repetidos são ignorados. A comparação prioriza título eleitoral, depois CPF e, quando esses campos não existem, nome + zona + seção.</p><div className="import-preview"><Table><TableHeader><TableRow><TableHead>#</TableHead><TableHead>Nome</TableHead><TableHead>Título</TableHead><TableHead>Zona</TableHead><TableHead>Seção</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{importRows.slice(0,100).map(row=><TableRow key={row.index}><TableCell>{row.index}</TableCell><TableCell>{row.data?shown(row.data.name):'—'}</TableCell><TableCell className="mono">{row.data?shown(row.data.title):'—'}</TableCell><TableCell>{row.data?.zone||'—'}</TableCell><TableCell>{row.data?.section||'—'}</TableCell><TableCell>{row.error?<span className="status-error">{row.error}</span>:row.duplicate?<span className="status-dup">Repetido</span>:<span className="status-ok">Pronto</span>}</TableCell></TableRow>)}</TableBody></Table>{importRows.length>100&&<p className="preview-more">Prévia dos 100 primeiros de {importRows.length}.</p>}</div></>}
   <div className="modal-footer import-footer"><span>Campos aceitos: nome, título, zona, seção, nascimento, município, UF, endereço, CPF, celular e local de votação.</span><div><Button type="button" variant="outline" disabled={importing} onClick={()=>setImportOpen(false)}>Cancelar</Button><Button type="button" disabled={importing||!importRows.some(r=>r.data&&!r.duplicate)} onClick={()=>void runImport()}>{importing?'Importando…':`Importar ${importRows.filter(r=>r.data&&!r.duplicate).length}`}</Button></div></div>
