...
import { Plus, Printer, ArrowDownAZ, ArrowUpAZ, UsersRound, Pencil, RefreshCw, ArrowLeft, FileText, ShieldCheck, X, Upload, FileJson, AlertTriangle, CheckCircle2, Check, CheckCheck, Trash2, Download, ChevronDown, Filter, SlidersHorizontal, Circle, ChevronRight } from 'lucide-react';
import { Card, DeiaCard } from './report-card';
...
import { blank, fields, mask, shown, states, validate, formatIndication, sortRecords, extractImportRecords, normalizeImportRecord, recordIdentity, isDeiaIndication, type Field, type Person, type RecordData, type SortKey } from '@/lib/records';
...
export default function Cadastros() {
    ...
    useEffect(() => {
        const onClickOutsideFilter = (e: MouseEvent) => { if (filterDropdownRef.current && !filterDropdownRef.current.contains(e.target as Node)) { setFilterMenuOpen(false); setHoveredIndication(null); } };
    ...
    const indicationFilterItems = useMemo(() => {
        const total = records.length, marked = records.filter(r => r.marked).length;
        const items: Array<{ key: string; label: string; total: number; marked: number; unmarked: number }> = [{ key: 'all', label: 'Todas as indicações', total, marked, unmarked: total - marked }];
        uniqueIndications.forEach(ind => {
            const m = records.filter(r => (r.indication || '').trim().toLowerCase() === ind.toLowerCase());
            const mk = m.filter(r => r.marked).length;
            items.push({ key: ind, label: ind, total: m.length, marked: mk, unmarked: m.length - mk });
        });
        if (hasUnassigned) {
            const m = records.filter(r => !(r.indication || '').trim());
            const mk = m.filter(r => r.marked).length;
            items.push({ key: 'none', label: 'Sem indicação', total: m.length, marked: mk, unmarked: m.length - mk });
        }
        return items;
    }, [records, uniqueIndications, hasUnassigned]);
    const filteredRecords = useMemo(() => {
        let list = records;
        if (selectedIndication === 'none') list = list.filter(p => !(p.indication || '').trim());
        else if (selectedIndication !== 'all') list = list.filter(p => (p.indication || '').trim().toLowerCase() === selectedIndication.toLowerCase());
        if (selectedMarkedFilter === 'marked') list = list.filter(p => !!p.marked);
        else if (selectedMarkedFilter === 'unmarked') list = list.filter(p => !p.marked);
        return list;
    }, [records, selectedIndication, selectedMarkedFilter]);
    const activeFilterLabel = useMemo(() => {
        let name = 'Todas as indicações';
        if (selectedIndication === 'none') name = 'Sem indicação';
        else if (selectedIndication !== 'all') name = selectedIndication;
        if (selectedMarkedFilter === 'marked') return `${name} · Marcados (${filteredRecords.length})`;
        if (selectedMarkedFilter === 'unmarked') return `${name} · Não marcados (${filteredRecords.length})`;
    ...
    const handleMouseEnterOptions = useCallback(() => { if (optionsTimeoutRef.current) { clearTimeout(optionsTimeoutRef.current); optionsTimeoutRef.current = null; } setOptionsMenuOpen(true); }, []);
    const handleMouseLeaveOptions = useCallback(() => { if (optionsTimeoutRef.current) clearTimeout(optionsTimeoutRef.current); optionsTimeoutRef.current = setTimeout(() => setOptionsMenuOpen(false), 220); }, []);
    useEffect(() => {
    ...
    async function save(e: React.FormEvent) {
        e.preventDefault(); setFormError(''); let data; try { data = validate(draft); } catch (e) { setFormError((e as Error).message); return; } setSaving(true);
    ...
    useEffect(() => {
        if (!printMode) return; let active = true; setPrintReady(false);
        void document.fonts.ready.then(() => {
            if (!active || !measure.current) return; const elements = Array.from(measure.current.children) as HTMLElement[]; const out: PrintItem[][] = []; let page: PrintItem[] = [];
            if (isDeia) {
                const startTop = 20, pageMaxHeight = 810, gap = 5; let currentTop = startTop;
                elements.forEach((el, i) => {
                    const height = el.getBoundingClientRect().height * 0.75;
                    if (page.length > 0 && (currentTop + height > pageMaxHeight || page.length >= 10)) { out.push(page); page = []; currentTop = startTop; }
                    page.push({ person: sorted[i], number: i + 1, top: currentTop }); currentTop += height + gap;
            ...
            setPages(out); setPrintReady(true);
        }); return () => { active = false; };
    ...
    return <>
                ...
                <section className="listing"><div className="list-toolbar"><div className="list-title"><UsersRound size={21} /><h2>Lista de cadastros</h2><span className="counter">{records.length}</span></div><div className="tools"><Button variant="ghost" size="icon" title="Atualizar lista" aria-label="Atualizar lista" disabled={loading} onClick={() => void load()}><RefreshCw className={loading ? 'spin' : ''} /></Button><Button variant="outline" disabled={!records.length || loading || !!error} onClick={() => setPrintMode(true)}><Printer /> Imprimir fichas</Button></div></div>
                    <div className="sortbar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            <span>Ordenar por</span><Select value={sort} onValueChange={v => setSort(v as SortKey)}><SelectTrigger aria-label="Ordenar por" className="sort-select"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(orderLabels).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent></Select><Button variant="ghost" size="icon" aria-label={desc ? 'Usar ordem crescente' : 'Usar ordem decrescente'} title={desc ? 'Ordem decrescente' : 'Ordem crescente'} onClick={() => setDesc(!desc)}>{desc ? <ArrowUpAZ /> : <ArrowDownAZ />}</Button><span className="sort-summary">{desc ? 'Ordem decrescente' : 'Ordem crescente'}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            <span>Filtrar por indicação</span>
                            <div ref={filterDropdownRef} className="filter-dropdown-container">
                                <button type="button" className="filter-trigger-btn" onClick={() => { setFilterMenuOpen(p => !p); setHoveredIndication(null); }} aria-expanded={filterMenuOpen}>
                                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '240px' }}><Filter size={14} color="#127c80" /><span style={{ fontWeight: 600 }}>{activeFilterLabel}</span></div>
                                    <ChevronDown size={14} style={{ color: '#64748b', transform: filterMenuOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s ease', flexShrink: 0 }} />
                                </button>
                                {filterMenuOpen && (<div className="filter-dropdown-menu" role="menu" onMouseLeave={() => setHoveredIndication(null)}>
                                    {indicationFilterItems.map(item => {
                                        const isSel = selectedIndication.toLowerCase() === item.key.toLowerCase(), hasSub = item.marked > 0;
                                        if (!hasSub) return <button key={item.key} type="button" className={`filter-item ${isSel ? 'active' : ''}`} role="menuitem" onClick={() => { setSelectedIndication(item.key); setSelectedMarkedFilter('all'); setFilterMenuOpen(false); setHoveredIndication(null); }} onMouseEnter={() => setHoveredIndication(null)}><div className="filter-item-content"><span>{item.label}</span><span className="filter-item-badge">({item.total})</span></div>{isSel && <Check size={14} />}</button>;
                                        const isHov = hoveredIndication === item.key;
                                        return <div key={item.key} className="filter-parent-item-container" onMouseEnter={() => setHoveredIndication(item.key)}>
                                            <button type="button" className={`filter-item ${isSel ? 'active' : ''}`} role="menuitem" onClick={() => { setSelectedIndication(item.key); setSelectedMarkedFilter('all'); setFilterMenuOpen(false); setHoveredIndication(null); }}><div className="filter-item-content"><span>{item.label}</span><span className="filter-item-badge">({item.total})</span></div><div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>{isSel && <Check size={14} />}<ChevronRight size={14} className="filter-item-arrow" /></div></button>
                                            {isHov && (<div className="filter-submenu" role="menu" onMouseEnter={() => setHoveredIndication(item.key)}>
                                                <div style={{ padding: '4px 10px 6px', fontSize: '11px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: '1px solid #f1f5f9' }}>{item.label}</div>
                                                <button type="button" className={`filter-subitem ${isSel && selectedMarkedFilter === 'all' ? 'active' : ''}`} role="menuitem" onClick={() => { setSelectedIndication(item.key); setSelectedMarkedFilter('all'); setFilterMenuOpen(false); setHoveredIndication(null); }}><div style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><UsersRound size={14} color="#127c80" /><span>Todos ({item.total})</span></div>{isSel && selectedMarkedFilter === 'all' && <Check size={14} />}</button>
                                                <button type="button" className={`filter-subitem ${isSel && selectedMarkedFilter === 'marked' ? 'active' : ''}`} role="menuitem" onClick={() => { setSelectedIndication(item.key); setSelectedMarkedFilter('marked'); setFilterMenuOpen(false); setHoveredIndication(null); }}><div style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><CheckCircle2 size={14} color="#059669" /><span style={{ color: '#047857' }}>Marcados ({item.marked})</span></div>{isSel && selectedMarkedFilter === 'marked' && <Check size={14} color="#059669" />}</button>
                                                {item.unmarked > 0 && <button type="button" className={`filter-subitem ${isSel && selectedMarkedFilter === 'unmarked' ? 'active' : ''}`} role="menuitem" onClick={() => { setSelectedIndication(item.key); setSelectedMarkedFilter('unmarked'); setFilterMenuOpen(false); setHoveredIndication(null); }}><div style={{ display: 'inline-flex', alignItems: 'center', gap: '7px' }}><Circle size={14} color="#94a3b8" /><span>Não marcados ({item.unmarked})</span></div>{isSel && selectedMarkedFilter === 'unmarked' && <Check size={14} />}</button>}
                                            </div>)}
                                        </div>;
                                    })}
                                </div>)}
                            ...
                            {records.length > 0 && (
                                <div style={{ display: 'inline-flex', gap: '6px', marginLeft: '4px' }}>
                                    {filteredRecords.some(r => !r.marked) && <Button variant="outline" size="sm" onClick={() => markAllRecords(true)} style={{ height: '32px', fontSize: '12px', color: '#0f766e' }}><CheckCheck size={14} style={{ marginRight: '4px' }} /> Marcar</Button>}
                                    {filteredRecords.some(r => r.marked) && <Button variant="ghost" size="sm" onClick={() => { if (window.confirm('Deseja realmente desmarcar os cadastros filtrados?')) markAllRecords(false); }} style={{ height: '32px', fontSize: '12px', color: '#64748b' }}>Desmarcar</Button>}
                                </div>
                            )}
                            {(selectedIndication !== 'all' || selectedMarkedFilter !== 'all') && <Button variant="ghost" size="sm" onClick={() => { setSelectedIndication('all'); setSelectedMarkedFilter('all'); }} style={{ fontSize: '12px', color: '#667c8a', padding: '0 8px', height: '32px' }}><X size={14} style={{ marginRight: '4px' }} /> Limpar</Button>}
                        </div>
                    </div>
        ...
        {printMode && <main className="print-view"><div className="print-toolbar"><Button variant="ghost" onClick={() => setPrintMode(false)}><ArrowLeft /> Voltar à lista</Button><div><strong>Prévia de impressão{isDeia ? ' · Modelo Deia' : ''}</strong><span>{sorted.length} fichas · {pages.length} páginas · {orderLabels[sort]}</span></div><Button disabled={!printReady} onClick={() => window.print()}><Printer /> Imprimir / Salvar PDF</Button></div><p className="print-help">Papel A4 · Escala 100% · Margens: nenhuma · Desative os cabeçalhos e rodapés do navegador e ative os gráficos de fundo.</p>
            <div className="print-measure" ref={measure} aria-hidden="true">{sorted.map((p, i) => isDeia ? <DeiaCard key={p.id} person={p} number={i + 1} /> : <Card key={p.id} person={p} number={i + 1} />)}</div>
            <div className="pages">{pages.map((items, i) => <section className={`paper ${isDeia ? 'paper-deia' : ''}`} key={i} aria-label={'Página ' + (i + 1)}>{!isDeia && <><div className="paper-stripe" /><header className="paper-header"><h1>Levantamento de títulos eleitorais</h1><p>Fichas individuais</p></header></>}{items.map(item => <div className="positioned-card" key={item.person.id} style={{ top: item.top + 'pt' }}>{isDeia ? <DeiaCard person={item.person} number={item.number} /> : <Card person={item.person} number={item.number} />}</div>)}{!isDeia && <div className="paper-footer" />}</section>)}</div>
        </main>}
    </>;
}
